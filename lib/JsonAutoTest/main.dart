...
import 'model/mago.dart';
import 'widget/details_mago.dart';

void main() {
  String magosFeiticos = """
{
  "magos": [
    {
  ...
  """;
  runApp(MagosApp(jsonMagos: magosFeiticos));
}

class MagosApp extends StatelessWidget {
  String jsonMagos;

  MagosApp({super.key, required this.jsonMagos});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MagosListPage(jsonMagos: jsonMagos),
    );
  }
}

class MagosListPage extends StatefulWidget {
  String jsonMagos;
  MagosListPage({super.key, required this.jsonMagos});

  @override
  _MagosListPageState createState() => _MagosListPageState();
}

class _MagosListPageState extends State<MagosListPage> {
  List<Mago> magos = [];
  ...
  void loadMagos() {
    final jsonData = jsonDecode(widget.jsonMagos);
    final List<dynamic> listaJson = jsonData['magos'];

    //Transformação de MAP para LIST
    setState(() {
      magos = listaJson.map((e) => Mago.fromJson(e)).toList();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Lista de Magos')),
      body:
          magos.isEmpty
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => DetailMago(mago: mago),
                        ),
                      );
                    },
                  );
